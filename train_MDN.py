...
#================================== Build the model =============================================================
    ...
    np.savetxt(output_location+'_training_'+outFile+'.txt', history.history['loss'])
    np.savetxt(output_location+'_validation_'+outFile+'.txt', history.history['val_loss'])
...
def mixture_density_loss(nb_components, target_dimension=2):
    ...
    def loss(y_true, y_pred):
        ...
        #logprob = T.switch(logprob >= 10.0 , 0.0, logprob) #Theano compatible
        logprob = tf.where(logprob >= 10.0, tf.zeros(tf.shape(logprob)), logprob) #Tensorflow compatible
        return logprob
